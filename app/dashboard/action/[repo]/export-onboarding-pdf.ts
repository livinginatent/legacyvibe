...
export function exportOnboardingToPDF(
  ...
  const metadata = `Generated: ${new Date().toLocaleDateString()}${
    data.userLevel ? ` | Level: ${data.userLevel.toUpperCase()}` : ""
  }${data.focusArea ? ` | Focus: ${data.focusArea}` : ""}`;
  ...
  doc.text(
    `Est. Time: ${Math.floor(data.estimatedTotalTime / 60)}h ${
      data.estimatedTotalTime % 60
    }m`,
    margin + 80,
    statsY
  );
  ...
  doc.text(
    `Progress: ${completedCount}/${data.totalSteps}`,
    margin + 10,
    statsY + 10
  );
  ...
  for (const step of data.learningPath) {
    ...
    if (step.checkpoints && step.checkpoints.length > 0) {
      ...
      for (const checkpoint of step.checkpoints) {
        checkPageBreak();
        const checkLines = doc.splitTextToSize(
          `  [ ] ${checkpoint}`,
          maxWidth - 10
        );
  ...
  doc.text("Generated by Cadracode Onboarding Copilot", pageWidth / 2, yPos, {
    align: "center",
  });
  yPos += 5;
  doc.text(
  ...
  const filename = `${repo}-onboarding${
